...
def warning(erreur_test, erreur_apprentissage, bruit):
    ...
    """
    # AJOUTER CODE ICI
    # pour le sur ou le sous apprentissage nous allons nous  base sur les données de test; les données qui vont nous  permettre de généraliser
    #notre modèle, et on les compare au bruit :
    # Si l'erreur sur l'ensemble de test est plus grande que le bruit, alors il y a un problème dans notre modèle
    if bruit < erreur_test:
        # nous allons comparé les données et le bruit pour voir  si c'est le overfitting ou le underfitting:
        if bruit > erreur_apprentissage:
            print("overfitting !")
        elif bruit < erreur_apprentissage:
            print("underfitting !")
